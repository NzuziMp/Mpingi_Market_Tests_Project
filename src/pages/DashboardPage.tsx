...
import { Plus, Eye, Trash2, CreditCard as Edit2, LayoutDashboard, Heart, User, Tag, TrendingUp, Clock, CheckCircle } from 'lucide-react';
...
export default function DashboardPage({ onNavigate }: DashboardPageProps) {
  ...
  async function loadSaved() {
    ...
    const { data } = await supabase
      ...
      .order('created_at', { ascending: false });
    if (data) {
      setSavedListings(data.map((d: any) => d.listings).filter(Boolean) as Listing[]);
    }
